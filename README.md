...
| monitors | The set of monitor specific attributes per monitor | <pre>map(object({<br>    name                      = string<br>    message                   = string<br>    monitor_threshold_windows = map(string)<br>    monitor_thresholds        = map(string)<br>    query                     = string<br>    thresholds                = map(string)<br>    type                      = string<br>  }))</pre> | `null` | no |
